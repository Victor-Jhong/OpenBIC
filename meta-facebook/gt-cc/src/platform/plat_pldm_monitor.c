		...
		*completion_code_p = PLDM_SUCCESS;
		...
		if (power_led_val_state->effecter_state == EFFECTER_STATE_PLAT_LED_VALUE_UNKNOWN) {
			*completion_code_p = PLDM_OEM_GPIO_EFFECTER_VALUE_UNKNOWN;
			...
			if (power_led_set(power_led_val, SYS_LED_USER_BMC) != 0) {
		...
		*completion_code_p = PLDM_SUCCESS;
		...
		if (fault_led_val_state->effecter_state == EFFECTER_STATE_PLAT_LED_VALUE_UNKNOWN) {
			*completion_code_p = PLDM_OEM_GPIO_EFFECTER_VALUE_UNKNOWN;
			...
			if (fault_led_set(fault_led_val, SYS_LED_USER_BMC) != 0) {
				*completion_code_p = PLDM_ERROR;
				return;
			};
			*completion_code_p = PLDM_SUCCESS;
...
void oem_set_effecter_type_plat_handler(const uint8_t *buf, uint16_t len, uint8_t *resp,
					uint16_t *resp_len)
{
	CHECK_NULL_ARG(buf);
	CHECK_NULL_ARG(resp);
	...
	*resp_len = 1;
	...
	ARG_UNUSED(completion_code_p);
	ARG_UNUSED(plat_effecter_id);
