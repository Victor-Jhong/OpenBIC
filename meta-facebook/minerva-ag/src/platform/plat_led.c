...
void lit_heartbeat_led(struct k_timer *timer)
{
	...
	for (size_t i = 0; i < num_sensors; i++) {
		...
		const sensor_val *reading_val = (const sensor_val *)&cache_reading;
		int sensor_reading = 0;
		if (reading_val->integer >= 0) {
			sensor_reading = reading_val->integer * 1000 + reading_val->fraction;
		} else {
			sensor_reading = reading_val->integer * 1000 - reading_val->fraction;
		}

		uint16_t candidate_interval = 500;
		if (sensor_reading <= 70000) { //70 degree C
			candidate_interval = 500;
		} else if (sensor_reading <= 100000) { //100 degree C
		...
		LOG_DBG("Sensor 0x%x reading: %d, candidate interval: %d ms", sensor_id,
			sensor_reading, candidate_interval);
	}
